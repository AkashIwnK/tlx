...
static Function *getIntrinsicDeclaration(CallInst *CI, std::vector<Type *> &ArgsTy,
  ...
  // Transpose
   ...
   // Padding
   if(CalledFuncName.contains(StringRef("tensor_pad"))) {
     errs() << "TENSOR PAD\n";
     auto *TypeInfoIntrinsic = FakeTypeToTokenTypeVal[CI->getArgOperand(0)];
     assert(dyn_cast<CallInst>(TypeInfoIntrinsic) && "Tensor ops' operands must come from typeinfo");
     return Intrinsic::getDeclaration(M, Intrinsic::tensor_pad,
                                    ArrayRef<Type*>({CI->getCalledFunction()->getReturnType(),
...
static bool isTensorCall(CallInst *CI) {
  ...
  return (CalledFuncName.contains(StringRef("tensor_typeinfo"))
       ...
       || CalledFuncName.contains(StringRef("tensor_pad"))
       );
...
SmallVector<Value *, 3> getPaddingOutputProperties(LLVMContext &Ctx,
                                                 SmallVector<Value *, 3> &Input,
                                                 SmallVector<unsigned, 3> &LowPads,
                                                 SmallVector<unsigned, 3> &HighPads,
                                                 SmallVector<unsigned, 3> &InteriorPads) {
    ...
    // Get the properties of the output tensor of padding op
    auto *Int32Ty = Type::getInt32Ty(Ctx);
    unsigned NumDims = dyn_cast<FixedVectorType>(Input[0]->getType())->getNumElements();
    std::vector<Constant *> ShapeVec;
    std::vector<Constant *> LayoutVec;
    ...
    for(unsigned i = 0; i < NumDims; i++) {
      unsigned LowPadding = LowPads[i];
      unsigned HighPadding = HighPads[i];
      unsigned InteriorPadding = InteriorPads[i];

      // Add padding
      unsigned DimVal = getVectorElemVal(Input[0], i);
      unsigned TotalPadding = (LowPadding + HighPadding + ((DimVal - 1) * InteriorPadding));
      ShapeVec.push_back(ConstantInt::get(Int32Ty, DimVal + TotalPadding));
      unsigned LayoutVal = getVectorElemVal(Input[1], i);
      LayoutVec.push_back(ConstantInt::get(Int32Ty, LayoutVal));
      unsigned PaddingVal = getVectorElemVal(Input[2], i);
      PaddingVec.push_back(ConstantInt::get(Int32Ty, PaddingVal + TotalPadding));
    }
...
static bool mapTensorValToProperty(Instruction *I, 
  ...
  if(auto *CI = dyn_cast<CallInst>(I)) {
    ...
    if(CalledFuncName.contains(StringRef("tensor_pad"))) {
     ...
     // Get low padding information
      SmallVector<unsigned, 3> LowPads;
      auto *LowPadsVal = CI->getArgOperand(2);
      auto *LowPadsVectorTy = dyn_cast<FixedVectorType>(LowPadsVal->getType());
      auto *LowPadsCV = dyn_cast<ConstantDataVector>(LowPadsVal);
      if(!LowPadsCV) {
        auto *LowPadsAZ = dyn_cast<ConstantAggregateZero>(LowPadsVal);
         for(unsigned I = 0; I < LowPadsVectorTy->getNumElements(); I++) {
            auto *C = LowPadsAZ->getElementValue(I);
            LowPads.push_back(dyn_cast<ConstantInt>(C)->getZExtValue());
         }
      } else {
          for(unsigned I = 0; I < LowPadsVectorTy->getNumElements(); I++) {
            auto *C = LowPadsCV->getAggregateElement(I);
            LowPads.push_back(dyn_cast<ConstantInt>(C)->getZExtValue());
          }
      }

      // Get high padding information
      SmallVector<unsigned, 3> HighPads;
      auto *HighPadsVal = CI->getArgOperand(3);
      auto *HighPadsVectorTy = dyn_cast<FixedVectorType>(HighPadsVal->getType());
      auto *HighPadsCV = dyn_cast<ConstantDataVector>(HighPadsVal);
      if(!HighPadsCV) {
        auto *HighPadsAZ = dyn_cast<ConstantAggregateZero>(HighPadsVal);
         for(unsigned I = 0; I < HighPadsVectorTy->getNumElements(); I++) {
            auto *C = HighPadsAZ->getElementValue(I);
            HighPads.push_back(dyn_cast<ConstantInt>(C)->getZExtValue());
         }
      } else {
        for(unsigned I = 0; I < HighPadsVectorTy->getNumElements(); I++) {
            auto *C = HighPadsCV->getAggregateElement(I);
            HighPads.push_back(dyn_cast<ConstantInt>(C)->getZExtValue());
        }
      }

      // Get interrior padding information
      SmallVector<unsigned, 3> InteriorPads;
      auto *InteriorPadsVal = CI->getArgOperand(4);
      auto *InteriorPadsVectorTy = dyn_cast<FixedVectorType>(InteriorPadsVal->getType());
      auto *InteriorPadsCV = dyn_cast<ConstantDataVector>(InteriorPadsVal);
      if(!InteriorPadsCV) {
        auto *InteriorPadsAZ = dyn_cast<ConstantAggregateZero>(InteriorPadsVal);
         for(unsigned I = 0; I < InteriorPadsVectorTy->getNumElements(); I++) {
          auto *C = InteriorPadsAZ->getElementValue(I);
          InteriorPads.push_back(dyn_cast<ConstantInt>(C)->getZExtValue());
         }
      } else {
        for(unsigned I = 0; I < InteriorPadsVectorTy->getNumElements(); I++) {
          auto *C = InteriorPadsCV->getAggregateElement(I);
          InteriorPads.push_back(dyn_cast<ConstantInt>(C)->getZExtValue());
        }
      }
      
      // Add the output tensor's properties
      ValToPropertyMap[CI] = getPaddingOutputProperties(CI->getModule()->getContext(), 
                                                  ValToPropertyMap[CI->getArgOperand(0)],
