...
#include "llvm/Transforms/Utils/TensorUtils.h"
#include "llvm/Transforms/Utils/TilingSupport.h"
...
//struct CodeGenKnobs {
  unsigned TileSize_M = 32;
  unsigned TileSize_N = 32;
  unsigned TileSize_K = 32;
...
public:
  LowerTensorIntrinsics(
      ...
      TensorInfo *TI) //DeviceCodeGen *DCG = nullptr)
  ...
  public:
    ...
    void insert2DTilePHIs(Type *ElemType, TensorInfo &TI) {
      ...
      BasicBlock *InnerLoopPreheader =
                    LoopNestInfo.LoopPreheaders[NumPreheaders - 1];
      auto *InnerLoopPreheaderTerminator = InnerLoopPreheader->getTerminator();
      auto *InnerHeaderTerminator = InnerLoopHeader->getTerminator();
      auto *M = InnerLoopPreheaderTerminator->getModule();
      ...
      for (unsigned I = 0; I < Num2DRegTileRows; I++) {
        for (unsigned J = 0; J < Num2DRegTileCols; J++) {
          // Insert broadcast intrinsic in loop preheader
          auto *Poison = PoisonValue::get(Type::getTokenTy(Ctx));
          auto *BroadcastIntrinsic = Intrinsic::getDeclaration(
                                      M, Intrinsic::tensor_broadcast, 
                                      ArrayRef<Type*>({TileVecTy}));
          std::vector<Value *> Args {Poison, GetConstantValue(Ctx, ElemType, 0)};
          auto *Broadcast = CallInst::Create(BroadcastIntrinsic, Args, "", 
                                InnerLoopPreheaderTerminator);
          TI.addTensorInfoFor(Poison, Out2DTileReg);
          TI.addTensorInfoFor(Broadcast, Out2DTileReg);
          ...
          Phi->addIncoming(
              //ConstantAggregateZero::get(TileVecTy), 
              Broadcast,
              InnerLoopPreheader);
  ...
  private:
    ...
    void setRegInfo(LLVMContext &Ctx) {
        SmallVector<unsigned, 4> ShapeVector {16, 16};
  ...
  Value *loadTensor(Value *Ptr,  Type *ElemTy, 
    ...
    return new LoadInst(VecTy, Ptr, "input.load", false, {}, InsertBefore);
  }
  ...
  void generateMatrixMultiply1DKernel(
    ...
    const unsigned VF = std::max<unsigned>(
        TTI.getRegisterBitWidth(TargetTransformInfo::RGK_FixedWidthVector).getFixedSize() / 
  ...
  void generateMatrixMultiply2DKernel(
    ...
    auto *MMAFunc = Intrinsic::getDeclaration(InsertBefore->getModule(), 
                    Intrinsic::tensor_smma, ArrayRef<Type *>({OutTileVecTy}));
  ...
  Value *lowerMatMulStage1(
    ...
    //generateMatrixMultiply2DKernelStage1(MMInfo, EltType, InnerBodyTerminator);
...
bool LowerTensorIntrinsicsLegacyPass::runOnFunction(Function &F) {
  ...
  //DeviceCodeGen DCG(F.getParent());
  LowerTensorIntrinsics LMT(F, TTI, &DT, &LI, &TI);//, &DCG);
