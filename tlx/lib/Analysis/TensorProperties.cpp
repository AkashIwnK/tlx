...
TensorType TensorInfo::getPaddingOutputProperties(LLVMContext &Ctx,
                                                 TensorType &Input,
...
bool TensorInfo::mapTensorValToProperty(Instruction *I, 
  ...
  if(auto *II = dyn_cast<IntrinsicInst>(I)) {
    ...
    if(II->getIntrinsicID() == Intrinsic::tensor_pad) {
      // If the input tensor value's propeties have not been resolved yet,
      // we will resolve them later.
      const auto &It = ValToPropertyMap.find(II->getArgOperand(0));
      errs() << "CALL OPERAND: " << *(II->getArgOperand(0)) << "\n";
      if(It == ValToPropertyMap.end()) {
      ...
      SmallVector<unsigned, 3> LowPads;
      auto *LowPadsVal = II->getArgOperand(2);
      ...
      SmallVector<unsigned, 3> HighPads;
      auto *HighPadsVal = II->getArgOperand(3);
      ...
      SmallVector<unsigned, 3> InteriorPads;
      auto *InteriorPadsVal = II->getArgOperand(4);
      ...
      // Add the output tensor's properties
      ValToPropertyMap[II] = getPaddingOutputProperties(II->getModule()->getContext(), 
                                                  ValToPropertyMap[II->getArgOperand(0)],
